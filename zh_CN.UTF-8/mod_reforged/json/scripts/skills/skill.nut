[
  {
    "key": "6d6462a9b0545e0437dcdd19fe4b1e2b133edd47e19320cec7e6446f8834b926",
    "original": " (Chance: ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "6d6462a9b0545e0437dcdd19fe4b1e2b133edd47e19320cec7e6446f8834b926",
    "original": " (Chance: ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and misses \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "6d6462a9b0545e0437dcdd19fe4b1e2b133edd47e19320cec7e6446f8834b926",
    "original": " (Chance: ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and misses \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "6d6462a9b0545e0437dcdd19fe4b1e2b133edd47e19320cec7e6446f8834b926",
    "original": " (Chance: ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "7a074425d7e05d68f15b4bf0f811f4e65260061973b6c659e92e605b20e530c9",
    "original": " AP[/color][/b] to use and builds up ",
    "translation": "",
    "context": "\"[i]Costs \" + this.isAffordableBasedOnAPPreview() ? \"[b][color=\" + this.Const.UI.Color.PositiveValue + \"]\" + this.getActionPointCost() : \"[b][color=\" + this.Const.UI.Color.NegativeValue + \"]\" + this.getActionPointCost() + \" AP[/color][/b] to use and builds up \" + this.isAffordableBasedOnFatiguePreview() ? \"[b][color=\" + this.Const.UI.Color.PositiveValue + \"]\" + this.getFatigueCost() : \"[b][color=\" + this.Const.UI.Color.NegativeValue + \"]\" + this.getFatigueCost() + \" Fatigue[/color][/b][/i]\\n\""
  },
  {
    "key": "adf871629725f9b35c78cf6fe5c3bce51342bdd6f04ed6256f43612ffb66ff6c",
    "original": " Fatigue[/color][/b][/i]\n",
    "translation": "",
    "context": "\"[i]Costs \" + this.isAffordableBasedOnAPPreview() ? \"[b][color=\" + this.Const.UI.Color.PositiveValue + \"]\" + this.getActionPointCost() : \"[b][color=\" + this.Const.UI.Color.NegativeValue + \"]\" + this.getActionPointCost() + \" AP[/color][/b] to use and builds up \" + this.isAffordableBasedOnFatiguePreview() ? \"[b][color=\" + this.Const.UI.Color.PositiveValue + \"]\" + this.getFatigueCost() : \"[b][color=\" + this.Const.UI.Color.NegativeValue + \"]\" + this.getFatigueCost() + \" Fatigue[/color][/b][/i]\\n\""
  },
  {
    "key": "49834ae32db16615ad94f96d62883115930665d8aded28f6283749cdddfdaed1",
    "original": " and hits ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity)"
  },
  {
    "key": "49834ae32db16615ad94f96d62883115930665d8aded28f6283749cdddfdaed1",
    "original": " and hits ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "f53ce490244a62680e25c6dd90c10ca87e974ae9bf16e834154ffa327a81f841",
    "original": " and misses ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and misses \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "10662a43c6c3153dfaa3f3d1f23b70fb1f40c2a1f948db3aee692a064108d362",
    "original": " and the shot goes astray and hits ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity)"
  },
  {
    "key": "10662a43c6c3153dfaa3f3d1f23b70fb1f40c2a1f948db3aee692a064108d362",
    "original": " and the shot goes astray and hits ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "f744666397e4c323ab544c8bb36c670c9fc0d0f24fccfc149dbf6152fa7481d1",
    "original": " and the shot goes astray and misses ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and misses \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "36c995c02ede9cbbc8cc1b4dadf1b6e56f54c5e672495511303559547667491f",
    "original": " damage to hitpoints[/color]",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_max + \" damage to hitpoints[/color]\""
  },
  {
    "key": "a929ead1a21c112730266cc944d34c86467e326aa3acdbfcd60c2b6e2e610078",
    "original": " has destroyed ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(user) + \" has destroyed \" + this.Const.UI.getColorizedEntityName(targetEntity) + \"'s shield\""
  },
  {
    "key": "b65a2ade333e803690f161b44f088a276ff449a49d33935a0fd7470c6c3a8ff0",
    "original": " has hit ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(user) + \" has hit \" + this.Const.UI.getColorizedEntityName(targetEntity) + \"'s shield for 1 damage\""
  },
  {
    "key": "3c6bfe0ec8131b0f7d827142664b74bc5abcf4bab4ff95f7a6ba6f9e58e1255a",
    "original": " uses ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity)"
  },
  {
    "key": "3c6bfe0ec8131b0f7d827142664b74bc5abcf4bab4ff95f7a6ba6f9e58e1255a",
    "original": " uses ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "3c6bfe0ec8131b0f7d827142664b74bc5abcf4bab4ff95f7a6ba6f9e58e1255a",
    "original": " uses ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and misses \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "3c6bfe0ec8131b0f7d827142664b74bc5abcf4bab4ff95f7a6ba6f9e58e1255a",
    "original": " uses ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and misses \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "3c6bfe0ec8131b0f7d827142664b74bc5abcf4bab4ff95f7a6ba6f9e58e1255a",
    "original": " uses ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "3c6bfe0ec8131b0f7d827142664b74bc5abcf4bab4ff95f7a6ba6f9e58e1255a",
    "original": " uses ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity)"
  },
  {
    "key": "b9a4dc379476d025dbc21689af9ddd8adbbe39b1e4fa53313bbba5845015e678",
    "original": " uses skill ",
    "translation": "",
    "context": "user.getName() + \" uses skill \" + this.getName()"
  },
  {
    "key": "003d25a73b3f2449cef9c0adccef0bfdf616c1a9ec8136084b1eedce36df8637",
    "original": "'s shield",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(user) + \" has destroyed \" + this.Const.UI.getColorizedEntityName(targetEntity) + \"'s shield\""
  },
  {
    "key": "9a4e57cc0e883b63685b2a6a56368016c5a51343a757504175dc5f7ce6b8d1b1",
    "original": "'s shield for 1 damage",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(user) + \" has hit \" + this.Const.UI.getColorizedEntityName(targetEntity) + \"'s shield for 1 damage\""
  },
  {
    "key": "c805d0166ab845a7a41678d10b9b45c52ac609e9d704234759989cdd0a9f4219",
    "original": ", Rolled: ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "c805d0166ab845a7a41678d10b9b45c52ac609e9d704234759989cdd0a9f4219",
    "original": ", Rolled: ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and the shot goes astray and misses \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "c805d0166ab845a7a41678d10b9b45c52ac609e9d704234759989cdd0a9f4219",
    "original": ", Rolled: ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and misses \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "c805d0166ab845a7a41678d10b9b45c52ac609e9d704234759989cdd0a9f4219",
    "original": ", Rolled: ",
    "translation": "",
    "context": "this.Const.UI.getColorizedEntityName(_user) + \" uses \" + this.getName() + \" and hits \" + this.Const.UI.getColorizedEntityName(_targetEntity) + \" (Chance: \" + this.Math.min(95, this.Math.max(5, toHit)) + \", Rolled: \" + rolled + \")\""
  },
  {
    "key": "11d0db4be322b321121842fcb3dcc1ab8516c2ee664b9ad5419f1b5e4aa60004",
    "original": "Armed with shield",
    "translation": "",
    "context": "text = \"Armed with shield\""
  },
  {
    "key": "1f9ee07072c8b75c7a1dce87c6d39ed31bfdb9165478637dda1a90884acd7e5d",
    "original": "Distance of ",
    "translation": "",
    "context": "\"Distance of \" + _targetTile.getDistanceTo(user.getTile())"
  },
  {
    "key": "bd65afc7c005a38822ed2d41eb1b0f108c0a079d66b5eec7336701ec1e6ddd53",
    "original": "Fast Adaption",
    "translation": "",
    "context": "text = \"Fast Adaption\""
  },
  {
    "key": "be3e2185d25da9d377df896a942dbc0e8e25acc6ea0fcd83b4672aad573c768d",
    "original": "Height advantage",
    "translation": "",
    "context": "text = \"Height advantage\""
  },
  {
    "key": "31614c7453f3647c1f021a322b14e53776ca42b50bb10f1c88deb17ad4ca247f",
    "original": "Height disadvantage",
    "translation": "",
    "context": "text = \"Height disadvantage\""
  },
  {
    "key": "09d82d3dbc5235f25cf3f2ff7fb6b7f8828389bb2078fb81fe0bc902c2506fc1",
    "original": "Immune to being disarmed",
    "translation": "",
    "context": "text = \"Immune to being disarmed\""
  },
  {
    "key": "88bc432d73dad6b27fea2cb0255fcb9ba9fed0dbcf7b8d7be39b79bb667fc415",
    "original": "Immune to being knocked back or hooked",
    "translation": "",
    "context": "text = \"Immune to being knocked back or hooked\""
  },
  {
    "key": "9e43ff3abcbbadef5fb5b9a46abdc442289cc3d542fb06f45c4282b502accfe0",
    "original": "Immune to being rooted",
    "translation": "",
    "context": "text = \"Immune to being rooted\""
  },
  {
    "key": "5071b7099b35d360f57c676a5e09881f6c906959d4c8573511a059ddb9cb226e",
    "original": "Immune to stun",
    "translation": "",
    "context": "text = \"Immune to stun\""
  },
  {
    "key": "9b108d29e9b317c57f050e6524f764c8a4c8dc0bf4a90d73a3dc62848463d2ce",
    "original": "Inflicts [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_max + \" damage to hitpoints[/color]\""
  },
  {
    "key": "9b108d29e9b317c57f050e6524f764c8a4c8dc0bf4a90d73a3dc62848463d2ce",
    "original": "Inflicts [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_max + \"[/color] damage that ignores armor\""
  },
  {
    "key": "9b108d29e9b317c57f050e6524f764c8a4c8dc0bf4a90d73a3dc62848463d2ce",
    "original": "Inflicts [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_armor_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_armor_max + \"[/color] damage to armor\""
  },
  {
    "key": "9b108d29e9b317c57f050e6524f764c8a4c8dc0bf4a90d73a3dc62848463d2ce",
    "original": "Inflicts [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_max + \"[/color] damage to hitpoints, of which [color=\" + this.Const.UI.Color.DamageValue + \"]0[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_max + \"[/color] can ignore armor\""
  },
  {
    "key": "b3ce8f082c5333a51874503e85870c7604ac10588202e218b47b937049351a97",
    "original": "Killed",
    "translation": "",
    "context": "KilledString = \"Killed\""
  },
  {
    "key": "7ef0d5f0e962e2a7bf1efd4b7ed84748532eab35a6535ecf7b91ac8500d84fb8",
    "original": "Line of fire blocked",
    "translation": "",
    "context": "text = \"Line of fire blocked\""
  },
  {
    "key": "8a1c3c535b5f8c27543d6bdd4628a4e9587fadc0899429bf8bef6c3f182c8192",
    "original": "Nighttime",
    "translation": "",
    "context": "text = \"Nighttime\""
  },
  {
    "key": "527f1db688dc9bb22c2cdabcb11656cf7b7c4ef9f07c8e2d1bef850757e32b0b",
    "original": "Oath of Wrath",
    "translation": "",
    "context": "text = \"Oath of Wrath\""
  },
  {
    "key": "fa4e8c7a85e564e82564e6abb7f7188ac47f9c09fd409d63fa7b6d117f98c329",
    "original": "On bad terrain",
    "translation": "",
    "context": "text = \"On bad terrain\""
  },
  {
    "key": "e8dbee3cf4d7677d113bdc3fb99790f04618e60dc895945ea6167c7f85fc595b",
    "original": "Resistance against piercing attacks",
    "translation": "",
    "context": "text = \"Resistance against piercing attacks\""
  },
  {
    "key": "27f25295a8c163c70f02ee507e492fbd372fb5f69c524144fd0f7c5b62fde203",
    "original": "Resistance against ranged weapons",
    "translation": "",
    "context": "text = \"Resistance against ranged weapons\""
  },
  {
    "key": "6e1bdc294002f23d867897b239a644801a07effca16fc07c31ab55c7f41ff26d",
    "original": "Riposte",
    "translation": "",
    "context": "text = \"Riposte\""
  },
  {
    "key": "7ddf6341e8dce5a8e3e2996de0f42d450b47dc9c9698d9337480c6b3a02b418e",
    "original": "Surrounded",
    "translation": "",
    "context": "text = \"Surrounded\""
  },
  {
    "key": "20044c737aa2a5a8491131efc7e3bfff7f720892db01efa8cf497f0d9c074cb7",
    "original": "Target on bad terrain",
    "translation": "",
    "context": "text = \"Target on bad terrain\""
  },
  {
    "key": "6690c1bc90a4df2ab95813e59eaed855635f0c840eec3bcc9f372eafe593b925",
    "original": "Too close",
    "translation": "",
    "context": "text = \"Too close\""
  },
  {
    "key": "e3284ede5a60c4a7bfa19f0c277b32d2ee9503aea9eabb32f775a55b6cf8d771",
    "original": "[/color] - [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_max + \" damage to hitpoints[/color]\""
  },
  {
    "key": "e3284ede5a60c4a7bfa19f0c277b32d2ee9503aea9eabb32f775a55b6cf8d771",
    "original": "[/color] - [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_max + \"[/color] damage that ignores armor\""
  },
  {
    "key": "e3284ede5a60c4a7bfa19f0c277b32d2ee9503aea9eabb32f775a55b6cf8d771",
    "original": "[/color] - [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_armor_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_armor_max + \"[/color] damage to armor\""
  },
  {
    "key": "e3284ede5a60c4a7bfa19f0c277b32d2ee9503aea9eabb32f775a55b6cf8d771",
    "original": "[/color] - [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_max + \"[/color] damage to hitpoints, of which [color=\" + this.Const.UI.Color.DamageValue + \"]0[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_max + \"[/color] can ignore armor\""
  },
  {
    "key": "3b5c1fa9f4855ae1d841127226a827ad8b09b72dcd69050a95f3c9e2ccd184a3",
    "original": "[/color] can ignore armor",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_max + \"[/color] damage to hitpoints, of which [color=\" + this.Const.UI.Color.DamageValue + \"]0[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_max + \"[/color] can ignore armor\""
  },
  {
    "key": "06fe37f4fd5a56117436aea9ca1eba780f5637fab53c0c379b49d2e611566667",
    "original": "[/color] damage that ignores armor",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_max + \"[/color] damage that ignores armor\""
  },
  {
    "key": "9d430eae66be62c67bd35c43bb969a22e15fb3f873cc5ec56ac3d45282283c79",
    "original": "[/color] damage to armor",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_armor_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_armor_max + \"[/color] damage to armor\""
  },
  {
    "key": "cdc465e9b38541d0293fef63d63d7a2df0d5b20476a0b26f5463e70f89987fdb",
    "original": "[/color] damage to hitpoints, of which [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_max + \"[/color] damage to hitpoints, of which [color=\" + this.Const.UI.Color.DamageValue + \"]0[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_max + \"[/color] can ignore armor\""
  },
  {
    "key": "869da709a73d3664e70ae2816e0058c67bb264ab214f008c967ca8736fa20e10",
    "original": "[b][color=",
    "translation": "",
    "context": "\"[i]Costs \" + this.isAffordableBasedOnAPPreview() ? \"[b][color=\" + this.Const.UI.Color.PositiveValue + \"]\" + this.getActionPointCost() : \"[b][color=\" + this.Const.UI.Color.NegativeValue + \"]\" + this.getActionPointCost() + \" AP[/color][/b] to use and builds up \" + this.isAffordableBasedOnFatiguePreview() ? \"[b][color=\" + this.Const.UI.Color.PositiveValue + \"]\" + this.getFatigueCost() : \"[b][color=\" + this.Const.UI.Color.NegativeValue + \"]\" + this.getFatigueCost() + \" Fatigue[/color][/b][/i]\\n\""
  },
  {
    "key": "ebc36c6d892598a40ef393a3420e5158358cd319a89513ee7dc6cbd555f9b114",
    "original": "[i]Costs ",
    "translation": "",
    "context": "\"[i]Costs \" + this.isAffordableBasedOnAPPreview() ? \"[b][color=\" + this.Const.UI.Color.PositiveValue + \"]\" + this.getActionPointCost() : \"[b][color=\" + this.Const.UI.Color.NegativeValue + \"]\" + this.getActionPointCost() + \" AP[/color][/b] to use and builds up \" + this.isAffordableBasedOnFatiguePreview() ? \"[b][color=\" + this.Const.UI.Color.PositiveValue + \"]\" + this.getFatigueCost() : \"[b][color=\" + this.Const.UI.Color.NegativeValue + \"]\" + this.getFatigueCost() + \" Fatigue[/color][/b][/i]\\n\""
  },
  {
    "key": "8797d8691e8e51651cb7aa2650dbd2b9d1d3f051666077a25e2668230b7984d7",
    "original": "]0[/color] - [color=",
    "translation": "",
    "context": "\"Inflicts [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_min + \"[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_regular_max + \"[/color] damage to hitpoints, of which [color=\" + this.Const.UI.Color.DamageValue + \"]0[/color] - [color=\" + this.Const.UI.Color.DamageValue + \"]\" + damage_direct_max + \"[/color] can ignore armor\""
  },
  {
    "key": "1040671dfcb8d4af8cfe37734678b44393e0600f40898015764e4ff1e2dffb5d",
    "original": "]Can not be used because this character has taken an oath precluding the use of ranged weapons or tools[/color]",
    "translation": "",
    "context": "\"[color=\" + this.Const.UI.Color.NegativeValue + \"]Can not be used because this character has taken an oath precluding the use of ranged weapons or tools[/color]\""
  }
]