[
  {
    "ID": 293421755,
    "key": "Poisoned",
    "original": "Poisoned",
    "translation": "中毒",
    "stage": 1,
    "context": "this.m.Name = 'Poisoned'"
  },
  {
    "ID": 293421756,
    "key": "[/color] more turn(s).",
    "original": "[/color] more turn(s).",
    "translation": "[/color] 回合。",
    "stage": 1,
    "context": "return 'This character has a vicious poison running through his veins and will lose [color=' + this.Const.UI.Color.NegativeValue + ']' + this.m.Damage + '[/color] hitpoints each turn for [color=' + this.Const.UI.Color.NegativeValue + ']' + this.m.TurnsLeft + '[/color] more turn(s).';"
  },
  {
    "ID": 293421757,
    "key": "Died from poison",
    "original": "Died from poison",
    "translation": "死于中毒",
    "stage": 1,
    "context": "this.m.KilledString = 'Died from poison'"
  },
  {
    "ID": 293421758,
    "key": "This character has a vicious poison running through his veins and will lose [color=",
    "original": "This character has a vicious poison running through his veins and will lose [color=",
    "translation": "这个角色体内流淌着剧毒，在战斗中会失去[color=",
    "stage": 1,
    "context": "return 'This character has a vicious poison running through his veins and will lose [color=' + this.Const.UI.Color.NegativeValue + ']' + this.m.Damage + '[/color] hitpoints each turn for [color=' + this.Const.UI.Color.NegativeValue + ']' + this.m.TurnsLeft + '[/color] more turn(s).';"
  },
  {
    "ID": 293421759,
    "key": "[/color] hitpoints each turn for [color=",
    "original": "[/color] hitpoints each turn for [color=",
    "translation": "[/color] 生命值, 持续 [color=",
    "stage": 1,
    "context": "return 'This character has a vicious poison running through his veins and will lose [color=' + this.Const.UI.Color.NegativeValue + ']' + this.m.Damage + '[/color] hitpoints each turn for [color=' + this.Const.UI.Color.NegativeValue + ']' + this.m.TurnsLeft + '[/color] more turn(s).';"
  }
]
