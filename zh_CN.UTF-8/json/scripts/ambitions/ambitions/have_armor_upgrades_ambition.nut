[
  {
    "ID": 293403456,
    "key": "Mundane armors won't do for a real mercenary outfit.\nWe should adorn our gear with trophies of our deeds!",
    "original": "Mundane armors won't do for a real mercenary outfit.\\nWe should adorn our gear with trophies of our deeds!",
    "translation": "世俗的盔甲不适合作为真正的雇佣兵装备。\\n我们应该用战利品来装饰我们的装备！",
    "stage": 1,
    "context": "this.m.ButtonText = \"Mundane armors won't do for a real mercenary outfit.\nWe should adorn our gear with trophies of our deeds!\""
  },
  {
    "ID": 293403457,
    "key": "This will serve us well.",
    "original": "This will serve us well.",
    "translation": "这对我们很有帮助。",
    "stage": 1,
    "context": "this.m.SuccessButtonText = 'This will serve us well.'"
  },
  {
    "ID": 293403458,
    "key": "Have at least 6 armors with attachments",
    "original": "Have at least 6 armors with attachments",
    "translation": "拥有至少6件带附件的盔甲",
    "stage": 1,
    "context": "this.m.UIText = 'Have at least 6 armors with attachments'"
  },
  {
    "ID": 293403459,
    "key": "Have at least 6 armors with attachments in your possession. Buy them, loot them, or have a taxidermist craft them for you, then combine them with the armor of your men.",
    "original": "Have at least 6 armors with attachments in your possession. Buy them, loot them, or have a taxidermist craft them for you, then combine them with the armor of your men.",
    "translation": "在你的所有中拥有至少6件带附件的盔甲。 购买它们，掠夺它们，或者有一个剥制师为你制作他们，然后将它们与你的人的盔甲结合。",
    "stage": 1,
    "context": "this.m.TooltipText = 'Have at least 6 armors with attachments in your possession. Buy them, loot them, or have a taxidermist craft them for you, then combine them with the armor of your men.'"
  },
  {
    "ID": 293403460,
    "key": "[img]gfx/ui/events/event_82.png[/img]When you took command of the company it was a ragtag band of men clinging for life, an ad hoc assembly of sellswords stitched together with nothing if not sheer stubbornness and absolute contempt for common sense. Now ",
    "original": "[img]gfx/ui/events/event_82.png[/img]When you took command of the company it was a ragtag band of men clinging for life, an ad hoc assembly of sellswords stitched together with nothing if not sheer stubbornness and absolute contempt for common sense. Now you watch as the men walk around like frayed revenants of an unworldly wild, their armor adorned with hideous scalps and pelts and bones, the ill-shapen recreation of conquest fitted such that others may know the %companyname% could not be sorted by any measure known to the earth whether it be monster or man.",
    "translation": "[img]gfx/ui/events/event_82.png[/img]当你接掌战队时，那只是一群苟且求生的乌合之众，一群临时拼凑起来的佣兵，除了顽固和对常识的绝对蔑视外，一无所有。 现在你看着这些人像一个超凡脱俗的野兽一样四处走动，他们的盔甲上装饰着丑陋的头皮、毛皮和骨头，这种不合时宜的征服者式的娱乐方式，使其他人知道 %companyname% 不能按照大陆上已知的任何标准来分类，无论是怪物还是人类。",
    "stage": 1,
    "context": "this.m.SuccessText = '[img]gfx/ui/events/event_82.png[/img]When you took command of the company it was a ragtag band of men clinging for life, an ad hoc assembly of sellswords stitched together with nothing if not sheer stubbornness and absolute contempt for common sense. Now you watch as the men walk around like frayed revenants of an unworldly wild, their armor adorned with hideous scalps and pelts and bones, the ill-shapen recreation of conquest fitted such that others may know the %companyname% could not be sorted by any measure known to the earth whether it be monster or man.'"
  }
]
