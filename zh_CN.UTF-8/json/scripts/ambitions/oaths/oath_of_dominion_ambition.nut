[
    {
        "key": "[img]gfx/ui/events/event_180.png[/img]There is no more common a menace in this world than that of the common beast. And yet, despite this rampant disease of creatures, very few men are willing to take up arms and go seeking their vanquishment. It is only you all, the %companyname%, who swore an oath to slay the monsters, and slay them you did. With steady hands and stilled hearts, you kept to that Oath.\n\nTriumphant over beasts and monsters, the men are ready to take on whatever's next!",
        "original": "[img]gfx/ui/events/event_180.png[/img]There is no more common a menace in this world than that of the common beast. And yet, despite this rampant disease of creatures, very few men are willing to take up arms and go seeking their vanquishment. It is only you all, the %companyname%, who swore an oath to slay the monsters, and slay them you did. With steady hands and stilled hearts, you kept to that Oath.\n\nTriumphant over beasts and monsters, the men are ready to take on whatever's next!",
        "translation": "[img]gfx/ui/events/event_180.png[/img]在这个世界上，没有比普通野兽更常见的威胁了。然而，尽管这种动物疾病猖獗，很少有人愿意拿起武器去追杀它们。只有你们所有人，%companyname%，发誓要击杀这些怪物，并且你们做到了。以稳定的手和平静的心，你们履行了这个誓言。\n\n在野兽和怪物面前战胜，这些人已经准备好去接下来的挑战了！",
        "context": "this.m.SuccessText = \"[img]gfx/ui/events/event_180.png[/img]There is no more common a menace in this world than that of the common beast. And yet, despite this rampant disease of creatures, very few men are willing to take up arms and go seeking their vanquishment. It is only you all, the %companyname%, who swore an oath to slay the monsters, and slay them you did. With steady hands and stilled hearts, you kept to that Oath.\\n\\nTriumphant over beasts and monsters, the men are ready to take on whatever's next!\""
    },
    {
        "key": "{For Young Anselm! | As Oathtakers! | And death to the Oathbringers!}",
        "original": "{For Young Anselm! | As Oathtakers! | And death to the Oathbringers!}",
        "translation": "{为了小安瑟姆！ | 宣誓者万岁！ | 誓约使者去死吧！}",
        "context": "this.m.SuccessButtonText = '{For Young Anselm! | As Oathtakers! | And death to the Oathbringers!}'"
    },
    {
        "key": "]-5[/color] Melee and Ranged Skill when fighting any other foe.",
        "original": "]-5[/color] Melee and Ranged Skill when fighting any other foe.",
        "translation": "]-5[/color] 近战和远程技能(与其他敌人战斗时)。",
        "context": "this.m.OathBurdenText = 'Your men have [color=' + this.Const.UI.Color.NegativeValue + ']-10[/color] Resolve and [color=' + this.Const.UI.Color.NegativeValue + ']-5[/color] Melee and Ranged Skill when fighting any other foe.'"
    },
    {
        "key": "Your men have [color=",
        "original": "Your men have [color=",
        "translation": "你的所有战团成员获得[color=",
        "context": "this.m.OathBurdenText = 'Your men have [color=' + this.Const.UI.Color.NegativeValue + ']-10[/color] Resolve and [color=' + this.Const.UI.Color.NegativeValue + ']-5[/color] Melee and Ranged Skill when fighting any other foe.'"
    },
    {
        "key": "You gain extra Renown for slaying beasts (",
        "original": "You gain extra Renown for slaying beasts (",
        "translation": "誓言目标: 击杀目标的野兽数目(",
        "context": "return 'You gain extra Renown for slaying beasts (' + this.getBonusObjectiveProgress() + '/' + this.getBonusObjectiveGoal() + ').';"
    },
    {
        "key": "]+20[/color] Resolve and [color=",
        "original": "]+20[/color] Resolve and [color=",
        "translation": "]+20[/color] 决心和 [color=",
        "context": "this.m.OathBoonText = 'Your men have [color=' + this.Const.UI.Color.PositiveValue + ']+20[/color] Resolve and [color=' + this.Const.UI.Color.PositiveValue + ']+10[/color] Melee and Ranged Skill when fighting beasts.'"
    },
    {
        "key": "Oath of Dominion",
        "original": "Oath of Dominion",
        "translation": "统御誓言",
        "context": "this.m.OathName = 'Oath of Dominion'"
    },
    {
        "key": "]-10[/color] Resolve and [color=",
        "original": "]-10[/color] Resolve and [color=",
        "translation": "]-10[/color] 决心和 [color=",
        "context": "this.m.OathBurdenText = 'Your men have [color=' + this.Const.UI.Color.NegativeValue + ']-10[/color] Resolve and [color=' + this.Const.UI.Color.NegativeValue + ']-5[/color] Melee and Ranged Skill when fighting any other foe.'"
    },
    {
        "key": "Beasts have always threatened civilization.\nLet us take an Oath of Dominion and stake our claim against the tides of nature!",
        "original": "Beasts have always threatened civilization.\nLet us take an Oath of Dominion and stake our claim against the tides of nature!",
        "translation": "饕餮猛兽们是人类文明永恒的威胁。\n让我们立下统御誓言，与大自然的浪潮相抗争！",
        "context": "this.m.ButtonText = 'Beasts have always threatened civilization.\\nLet us take an Oath of Dominion and stake our claim against the tides of nature!'"
    },
    {
        "key": "\"We are from the beasts, but the beasts wish to have us back. Distance oneself from nature's primitiveness, prove your worth over Her such that your humanity be held in the grip of your own hands and seen through thine own eyes.\" - Young Anselm",
        "original": "\"We are from the beasts, but the beasts wish to have us back. Distance oneself from nature's primitiveness, prove your worth over Her such that your humanity be held in the grip of your own hands and seen through thine own eyes.\" - Young Anselm",
        "translation": "“我们源自野兽，但野兽也希望我们回归。远离自然的原始状态，证明自己胜过自然，让自己的人性握在自己的手中，透过自己的双眼看到。”-年轻的安瑟姆",
        "context": "this.m.TooltipText = '\"We are from the beasts, but the beasts wish to have us back. Distance oneself from nature\\'s primitiveness, prove your worth over Her such that your humanity be held in the grip of your own hands and seen through thine own eyes.\" - Young Anselm'"
    },
    {
        "key": "]+10[/color] Melee and Ranged Skill when fighting beasts.",
        "original": "]+10[/color] Melee and Ranged Skill when fighting beasts.",
        "translation": "]+10[/color] 近战和远程技能(与野兽战斗时)",
        "context": "this.m.OathBoonText = 'Your men have [color=' + this.Const.UI.Color.PositiveValue + ']+20[/color] Resolve and [color=' + this.Const.UI.Color.PositiveValue + ']+10[/color] Melee and Ranged Skill when fighting beasts.'"
    },
    {
        "key": "Your men have [color=",
        "original": "Your men have [color=",
        "translation": "你的所有战团成员获得[color=",
        "context": "this.m.OathBoonText = 'Your men have [color=' + this.Const.UI.Color.PositiveValue + ']+20[/color] Resolve and [color=' + this.Const.UI.Color.PositiveValue + ']+10[/color] Melee and Ranged Skill when fighting beasts.'"
    }
]